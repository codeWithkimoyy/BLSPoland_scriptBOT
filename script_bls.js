        ...
        (function() {
                ...
                panel.innerHTML = `
                    <div class="bls-panel" style="position:fixed;top:20px;right:20px;width:320px;max-height:80vh;background:white;border:2px solid #007bff;border-radius:10px;box-shadow:0 4px 20px rgba(0,0,0,0.15);z-index:10000;font-family:Arial,sans-serif;padding:15px;overflow-y:auto;overflow-x:hidden;">
                        ...
                        <div style="margin-bottom:10px;">
                            ...
                            <div id="error-banner" style="display:none;margin-top:8px;padding:6px;border-radius:4px;background:#f8d7da;color:#721c24;border:1px solid #f5c6cb;font-size:12px;"></div>
                        </div>
            ...
            function initGlobalErrorHandling() {
                ...
                window.addEventListener('unhandledrejection', (event) => {
                    const msg = event?.reason?.message || (typeof event?.reason === 'string' ? event.reason : 'Unhandled promise rejection');
        ...
        // Complete BLS form automation - Direct to Date Scanning
        async function startBLSFormAutomation() {
            try {
                log('🚀 Starting BLS form automation - Direct to Date Scanning...');

                // Skip Application Center, Service Type, and Application Type selection
                // Go directly to date scanning
        ...
        async function fillAllFormSections() {
                log('⚡ DIRECT MODE: Fill → Upload → Book Now (No Verification)');
                ...
                try {
                    ...
                    if (appointmentTypeSelect) {
                        ...
                        if (normalTimeSelected) {
                            // Step 3: Wait briefly for form fields to appear
            ...
            // Observe DOM for appointment type dropdown and auto-select Normal Time when it appears
            (function observeAppointmentTypeDropdown() {
                try {
                    const observer = new MutationObserver(async () => {
                        const select = document.querySelector('#valAppointmentType, select[name*="appointment_type"], select[id*="appointment_type"], select[class*="appointment"]');
                        if (select && select.options && select.options.length > 0) {
                            const ok = await selectNormalTimeAppointment(select);
                            if (ok) {
                                log('⚡ Auto-selected Normal Time as soon as dropdown appeared');
                                // Optionally stop observing if selection succeeded
                                observer.disconnect();
                            }
                        }
                    });
            ...
            async function fillFormWithEnhancedApproach() {
                log('⚡ DIRECT FILLING: No verification, no delays');
                ...
                // Build an ordered list of fields by DOM position so we fill top-to-bottom without jumping
                ...
                for (const [fieldName, fieldConfig] of Object.entries(fieldMappings)) {
                    ...
                    if (matchedElement) {
                        orderedEntries.push({ fieldName, fieldConfig, element: matchedElement });
                    }
                }

                // Sort by DOM order (first element in the document comes first)
                ...
                log(`🧭 Filling fields in DOM order (first to last). Total mappable fields: ${orderedEntries.length}`);

                for (const entry of orderedEntries) {
                    const { fieldName, fieldConfig, element } = entry;
                    ...
                    // If pre-filled, move on but preserve sequential traversal
                    ...
                    if (fieldConfig.type === 'select') {
                        success = await fillSelectField(element, profileValue, fieldName);
                    } else {
                        success = await fillInputField(element, profileValue, fieldName);
                    }
                ...
                log(`📊 Enhanced Form Filling Results (DOM order): ${filledCount} filled, ${errorCount} errors`);
            ...
            async function handleDateInput(element, value, fieldName) {
                try {
                    ...
                    // Set the new value directly (faster than simulating typing)
                    element.value = value;
                    element.dispatchEvent(new Event('input', { bubbles: true }));
                    element.dispatchEvent(new Event('change', { bubbles: true }));

                    // Confirm and finalize quickly
                    ...
                    log(`✅ Set date value for ${fieldName}: ${value}`);
                    return true;
                } catch (error) {
            ...
            async function handleSpecialFields() {
                ...
                // final aggressive pass removed to avoid re-filling and preserve strict order
        ...
        // finalAggressiveAttempt removed to keep sequential, non-jumping behavior
        ...
        // waitForFormSection removed (unused)

        // evaluateXPath and findFieldByLabelText removed (unused)

        // fillProblematicFields removed (unused)
        ...
        async function clickBookNowButton() {
                log('⏳ BOOK NOW: Adding 2-second delay before submit...');

            try {
                    // Add 5-second delay before clicking submit
                    log('⏳ Waiting 2 seconds before clicking submit button...');
                    await delay(2000); // 5 seconds delay
                    log('✅ 5-second delay completed, proceeding to click submit button');
                ...
                if (bookNowButton) {
                    bookNowButton.click();
                    log('✅ Book Now button clicked after 2-second delay!');
        ...
        // selectNormalTime removed (unused)
